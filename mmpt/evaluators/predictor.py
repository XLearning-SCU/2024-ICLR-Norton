    ...
    def predict_loop(
        ...
        with torch.no_grad():
            for data in eval_dataloader:
                ...
                # save video_id for video-paragraph retrieval
                if 'didemo' in self.config.dataset.meta_processor.lower():
                    outputs["video_id"] = data['video_id']
                    outputs['timestamps'] = data['timestamps']
                elif 'youcook' in self.config.dataset.meta_processor.lower():
...
class COINZSPredictor(COINPredictor):
    ...
    def predict_loop(self, model, eval_dataloader, output_file="result.pkl"):
        ...
        with torch.no_grad():
            ...
            label_hidden_states = model.forward_text(**outputs)[0].cpu()
    ...
    def predict_loop(self, model, eval_dataloader, output_file="didemo.npy"):
        ...
        with torch.no_grad():
            for data in eval_dataloader:
                ...
                pooled_text = model.forward_text(**data)[0]
                outputs = {
