...
class RetriTask(Task):
    ...
    def random_candidates(self):
        return self.train_data.meta_processor.random_candidates()
...
class VideoRetriTask(RetriTask):
    ...
    def _retri_sync(self, epoch, out_dir):
        ...
        batched_videos = []
        if self.config.vectorpool_cls == 'DistributedVideoVectorPool':
            fn = os.path.join(
                out_dir,
                "batched_e" + str(epoch) + "_videos" + str(0) + ".pkl")
...
class VideoPredictor(Predictor):
    ...
    def finalize(self):
        if isinstance(self.vecpool, vectorpool.DistributedVideoVectorPool):
            self.vecpool.build_retriver()
