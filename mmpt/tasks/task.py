...
class Task(object):
    ...
    def build_model(self, checkpoint=None):
        ...
        if self.config.model.layernorm_only == True:
    ...
    def load_mlp(self):
        ...
        load_state_dict = torch.load('./pretrained_models/s3d_fc.pth')
        state_dict = {k: v for k, v in load_state_dict.items() if k in model_dict.keys()}
    ...
    def build_loss(self):
        if self.loss_fn is None and self.config.loss is not None:
            ...
            self.loss_fn = loss_cls(self.config)
        return self.loss_fn
