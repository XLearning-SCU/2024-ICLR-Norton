...
class ShardedHow2VideoRetriMetaProcessor(ShardedHow2MetaProcessor):
    ...
    def random_candidates(self):
        ...
        self.cands = [
            original_data[batch_offset:batch_offset + self.num_video_per_batch]
            for batch_offset in
            range(0, (len(original_data) // (8 * self.num_video_per_batch)) * 8 * self.num_video_per_batch,
                  self.num_video_per_batch)]
