...
class VectorPool(object):
    ...
    def __init__(self, config):
        ...
        self.centroids = config.centroids if config.centroids is not None else 512
        self.examples_per_cent_to_train = config.examples_per_cent_to_train if config.examples_per_cent_to_train is not None else 48
    ...
    def build_retriver(
        ...
        centroids=None,  # 512
        db_type="flatl2",
        examples_per_cent_to_train=None # 48
    ):

        """merge results from multiple gpus and return a retriver.."""
        self.retriver = retriever_cls(
            hidden_size, self.centroids, db_type, self.examples_per_cent_to_train)
...
class DistributedVectorPool(VectorPool):
    ...
    def __init__(self, config):
        ...
        self.hidden_states = []
        self.video_ids = []
        self.centroids = config.centroids if config.centroids is not None else 4096
        self.examples_per_cent_to_train = config.examples_per_cent_to_train if config.examples_per_cent_to_train is not None else 48
