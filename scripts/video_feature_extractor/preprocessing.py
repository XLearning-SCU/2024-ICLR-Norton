...
class Preprocessing(object):
    ...
    def _zero_pad(self, tensor, size):
        ...
        if n == size:
            ...
            z = th.zeros(n, tensor.shape[1], tensor.shape[2], tensor.shape[3]) # n x 3 x H x W
            return th.cat((tensor, z), 0)

    def __call__(self, tensor, config):
        if self.type == '2d':
            tensor = tensor / 255.0
            ...
            tensor = tensor / 255.0
            tensor = self._zero_pad(tensor, config["fps"])
            tensor = tensor.view(-1, config["fps"], 3, 224, 224)  # N x fps x 3 x H x W
